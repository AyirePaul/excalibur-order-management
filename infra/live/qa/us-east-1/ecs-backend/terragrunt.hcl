...
dependency "ecr" {
  config_path = "../ecr"
  mock_outputs = { repository_urls = { backend = "000000000000.dkr.ecr.us-east-1.amazonaws.com/mock:latest" } }
...
inputs = {
  ...
  environment_vars = [
    ...
    { name = "ENABLE_DOCS",          value = tostring(local.env_vars.locals.enable_swagger) },
