...
TASK_DEF=$(terraform -chdir=infra output -raw backend_task_def_arn)
...
ansible-playbook -i inventories/dev/hosts.yml playbooks/deploy.yml \
  --extra-vars "$(jq -nc \
    ...
    --arg t "latest" \
    --arg td "$TASK_DEF" \
    '{ecs_cluster:$c, alb_url:$a, migrate_subnets:$s, migrate_security_groups:[$sg], image_tag:$t, migrate_task_definition:$td}')"
```
